...
/** Give untrusted installed dependencies no access to release or provider credentials. */
export function cliVerificationEnvironment(
  overrides: Record<string, string | undefined> = {},
  source: Record<string, string | undefined> = process.env,
): Record<string, string | undefined> {
  const environment: Record<string, string | undefined> = {};
  for (const key of CLI_VERIFICATION_ENV_KEYS) {
...
export async function installCliArchive(archive: string, destination: string): Promise<string> {
  ...
  const npmConfig = join(destination, '.npmrc');
  await writeFile(
    npmConfig,
  ...
  await execa(
    'npm',
    ['install', resolve(archive), '--omit=dev', '--ignore-scripts', '--no-audit', '--no-fund'],
    {
      cwd: destination,
      env: cliVerificationEnvironment({ NPM_CONFIG_USERCONFIG: npmConfig }),
      extendEnv: false,
      timeout: 120_000,
    },
  );
...
export async function smokeCliArchive(
  ...
  try {
    ...
    const { stdout: version } = await execa(process.execPath, [executable, '--version'], {
      cwd: temporary,
      env: cliVerificationEnvironment(),
      extendEnv: false,
      timeout: 10_000,
    });
    ...
    for (const command of [
      ...
      const { stdout } = await execa(process.execPath, [executable, ...command, '--help'], {
        cwd: temporary,
        env: cliVerificationEnvironment({ VIDEO_CLIPPER_API_URL: 'http://127.0.0.1:1' }),
        extendEnv: false,
        timeout: 10_000,
      });
    ...
    const result = await execa(shim, ['--version'], {
      cwd: temporary,
      env: cliVerificationEnvironment(),
      extendEnv: false,
      timeout: 10_000,
