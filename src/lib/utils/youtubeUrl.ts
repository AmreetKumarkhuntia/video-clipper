 ...
 *
 * It lives in `utils` rather than the video service because it depends on
 * nothing but the URL class, and all three apps need it: the CLI turns a
 * pasted url into the id the backend addresses videos by, without importing a
 * domain service to do it.
 *
...
/**
 * The same parse, for callers that treat "not a YouTube video url" as an
 * ordinary answer rather than a failure — a CLI reporting its own message, say.
 */
export function tryParseVideoId(url: string): string | null {
  try {
