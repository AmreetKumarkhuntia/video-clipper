...
| [routes/videos/[videoId]/+layout.svelte](../../../src/app/web/routes/videos/[videoId]/+layout.svelte) — workflow stepper                                      | `main.jsx` left rail `Pipeline`                                                        | Restyle [VideoWorkflowStepper.svelte](../../../src/app/web/widgets/video/analysis/VideoWorkflowStepper.svelte) using `.pipeline` + `.pipeline__step`. Status mapping: `complete→done`, `current→running`, `upcoming→pending`, `locked→pending` (with subtle lock affordance). |
