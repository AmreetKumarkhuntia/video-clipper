...
| Storage      | PostgreSQL, with tenant ownership still incomplete; media on local disk                                  | tenant scoping on every table; per-customer media paths           |
...
- `initDb(config)` owns the PostgreSQL pool, the db barrel hides the raw handle, and migrations are operator-run.
...
| D3  | Database                            | **superseded:** PostgreSQL-only, with one backend-owned pool and operator-controlled migrations                                                                                                                                                                                                                                                                                 | supports safe concurrency and production lifecycle controls without a file-database compatibility layer                                                      |
...
youtube_auth     (customer_id PK, access_token, refresh_token, expiry_date, scope,
                  channel_id, connected_at, updated_at)
customer_settings(customer_id PK, settings_json, updated_at)
...
- In-process worker started from the backend: polls `jobs`, claims atomically with a PostgreSQL row lock, runs with `p-limit`, writes throttled progress, retries with backoff.
...
- **Transaction duration**: workers must not hold PostgreSQL transactions around LLM calls.
