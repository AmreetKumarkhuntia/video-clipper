    ...
    {steps.map((s, i) => (
      <div key={i} className={`pipeline__step is-${s.state}`}>
        <div className="pipeline__icon">
          {s.state === "done" ? <Icons.Check size={12} style={{ strokeWidth: 2.5 }} />
            : s.state === "running" ? <Icons.Loader size={12} style={{ strokeWidth: 2 }} />
            : <span>{i + 1}</span>}
        </div>
...
const SegmentRow = ({ rank, start, end, dur, title, reason, score, active, cut, cutNum }) => (
  <div className={`segment ${active ? "is-active" : ""}`}>
    ...
    <div className="segment__time">{start} → {end}<span className="dur">{dur}</span></div>
    ...
    <div className="segment__right">
      ...
      {active ? (
        <button className="vc-btn vc-btn--primary vc-btn--sm"><Icons.Scissors size={14} />Cut clip</button>
      ) : (
        <button className="vc-btn vc-btn--secondary vc-btn--sm"><Icons.Play size={14} />Play</button>
      )}
    </div>
  </div>
);

const Player = ({ title = "How to ship great products without protecting every legacy interface", duration = "30:23" }) => (
...
const Timeline = ({ segments, currentPct = 14, current = "02:14", duration = "30:23", cutCount = 4 }) => (
  <div className="timeline">
    <div className="timeline__head">
      <div><span className="now">{current}</span> <span style={{ marginLeft: 8 }}>/ {duration}</span></div>
      <div>{segments.length} segments · {cutCount} cut</div>
    </div>
...
const SAMPLE_SEGMENTS = [
  ...
  { rank: "02", start: "02:14", end: "02:48", dur: "34s", title: "Strong controversial opinion", reason: "“You can't ship great products and protect every legacy interface.”", score: 9, active: true },
  { rank: "03", start: "06:42", end: "07:14", dur: "32s", title: "Audience reaction · laugh", reason: "audio event · spike at 07:01", score: 8 },
  { rank: "04", start: "09:36", end: "10:18", dur: "42s", title: "Counter-argument from the host", reason: "debate moment · transcript + audio", score: 8, cut: true, cutNum: "02" },
  { rank: "05", start: "13:22", end: "13:54", dur: "32s", title: "Reframe of the original question", reason: "key insight · transcript", score: 8 },
  { rank: "06", start: "16:08", end: "16:35", dur: "27s", title: "Callback to the cold open", reason: "narrative payoff · transcript", score: 7 },
];

const SAMPLE_TIMELINE = [
  { left: 4,  width: 5 }, { left: 11, width: 6, active: true }, { left: 22, width: 4 },
  { left: 32, width: 7, cut: true }, { left: 44, width: 4 }, { left: 53, width: 5 },
  { left: 63, width: 6 }, { left: 75, width: 4, cut: true }, { left: 84, width: 5, cut: true },
  ...
  <div className="screen-root" data-theme={theme}>
    ...
    <div className="shell">
      ...
      <aside className="rail">
        ...
        <Pipeline steps={[
          { name: "Fetch transcript", t: "1.2s", state: "done", detail: "youtube-transcript-api" },
        ...
        <div className="rail__group">
          <div className="rail__row"><span className="label">Duration</span><span className="val">30:23</span></div>
          <div className="rail__row"><span className="label">Chunks</span><span className="val">22</span></div>
          <div className="rail__row"><span className="label">Segments</span><span className="val">10</span></div>
          <div className="rail__row"><span className="label">Clips cut</span><span className="val">4</span></div>
          <div className="rail__row"><span className="label">Cache</span><span className="val">hit</span></div>
        </div>
        <button className="vc-btn vc-btn--secondary" style={{ width: "100%", marginTop: 12 }}>
          <Icons.Download size={14} />Download all clips
        </button>
      </aside>
      ...
      <div className="main">
        ...
        <div className="sect">
          ...
          <div className="segments">
            {SAMPLE_SEGMENTS.map((s) => <SegmentRow key={s.rank} {...s} />)}
          </div>
        </div>
      </div>
      ...
      <aside className="side">
        ...
        <div className="side__group">
          ...
          <div className="vc-field">
            ...
            <input type="number" className="vc-input" defaultValue="10" style={{ fontFamily: "var(--vc-font-mono)" }} />
          </div>
          <div className="vc-field">
          ...
          <label className="toggle-row">
            ...
            <span className="vc-toggle"><input type="checkbox" defaultChecked /><span className="vc-toggle__track"></span><span className="vc-toggle__thumb"></span></span>
          </label>
          <label className="toggle-row">
            Refine boundaries
            <span className="vc-toggle"><input type="checkbox" defaultChecked /><span className="vc-toggle__track"></span><span className="vc-toggle__thumb"></span></span>
        ...
        <div className="side__group">
          <div className="rail__row"><span className="label">Format</span><span className="val">mp4</span></div>
          <div className="rail__row"><span className="label">Aspect</span><span className="val">9:16</span></div>
          <div className="rail__row"><span className="label">Folder</span><span className="val val--ellipsis">~/clips/ship</span></div>
        </div>
      </aside>
    </div>
  </div>
