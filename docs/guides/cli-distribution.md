...
A push to `master` invokes the reusable CI workflow and waits for its type, format, Vitest, build, browser, and CLI-package checks. The package matrix covers Node 22/24 on Linux, macOS, and Windows. Release jobs are serialized.

The release job checks out the same source SHA, builds the CLI, and runs semantic-release. Its npm plugin stamps only `artifacts/cli/package.json`. The local release plugin then prepares and verifies both final archives. A recorded dirty build or a changed source commit/executable blocks release. The changelog/tag and GitHub release are still handled by semantic-release; no post-build pull or root package rename occurs.
