...
```bash
vdclip https://youtube.com/watch?v=VIDEO_ID --clip --video-path ./my-clips
```
...
vdclip https://youtube.com/watch?v=VIDEO_ID --clip --local-video /path/to/video.mp4
```
...
```bash
TIMESTAMP_OFFSET_SECONDS=-3 vdclip <url> --clip --local-video /path/to/video.mp4
```
...
OPENAI_API_KEY=sk-... LLM_PROVIDER=openai \
  npx vdclip https://youtube.com/watch?v=VIDEO_ID
```
