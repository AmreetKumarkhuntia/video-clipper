...
```bash
# Start the backend with faster processing (lower quality)
FFMPEG_PRESET=ultrafast pnpm api:dev

# Or start it with higher quality (slower)
FFMPEG_PRESET=medium pnpm api:dev

# Run the CLI separately; rendering uses the backend setting
vdclip <url> --clip
```
