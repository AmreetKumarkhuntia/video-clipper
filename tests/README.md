...
```bash
pnpm db:up
docker compose exec postgres createdb -U video_clipper video_clipper_test
TEST_DATABASE_URL=postgresql://video_clipper:video_clipper@127.0.0.1:5432/video_clipper_test pnpm test:integration
```

Create the test database once per fresh Compose volume; `createdb` reports that it already exists on
later runs. The safety check rejects URLs whose database name does not contain `test`.
