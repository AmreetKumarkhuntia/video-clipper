...
import {
  ...
  assertCliInputs,
  cliVerificationEnvironment,
} from '../../../../scripts/cliPackage.js';
...
describe('CLI distribution boundaries', (): void => {
  ...
  it('removes release and provider credentials from package verification processes', (): void => {
    expect(
      cliVerificationEnvironment(
        { VIDEO_CLIPPER_API_URL: 'http://127.0.0.1:1' },
        {
          PATH: '/test/bin',
          GITHUB_TOKEN: 'github-secret',
          NPM_TOKEN: 'npm-secret',
          GH_PACKAGES_TOKEN: 'packages-secret',
          OPENAI_API_KEY: 'provider-secret',
        },
      ),
    ).toEqual({ PATH: '/test/bin', VIDEO_CLIPPER_API_URL: 'http://127.0.0.1:1' });
  });
});
