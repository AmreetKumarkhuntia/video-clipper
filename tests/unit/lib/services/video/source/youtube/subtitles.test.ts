...
import {
  ...
  rankSubtitleFiles,
} from '@lib/services/video/source/youtube/subtitles.js';
...
describe('fetchTranscript yt-dlp fallback', () => {
  it('selects mhtml before reading the fallback subtitle file', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(
    ...
    execaMock.mockImplementation(async (_command: string, args: string[]): Promise<void> => {
      const outputIndex = args.indexOf('--output');
      const outputTemplate = args[outputIndex + 1];
      if (!outputTemplate) throw new Error('Missing yt-dlp output template');
      await fs.writeFile(path.join(path.dirname(outputTemplate), 'video.en.vtt'), BASIC_VTT);
    });
